...
<body class="colecao-module">
    ...
    <div class="content">
        ...
        <form method="POST" action="index.php?url=loja" id="formAdicionarColecao">
            <input type="hidden" name="action" value="create">

            <div id="edicaoPaginaBody">
                <div class="edit-modal-header-row" style="margin: 0px;">
                    <img id="edicaoImg" class="edit-modal-capa" src="assets/images/placeholder.jpg" alt="Capa do álbum">
                    <div class="edit-field-group" style="margin-bottom: 00px;">
                        <label>URL DA CAPA</label>
                        <input type="text" name="capa_url" id="edicaoCapaUrl" value="<?= htmlspecialchars($album['capa_url'] ?? '') ?>" placeholder="https://...">
                    </div>
                </div>
                ...
                <div class="edit-modal-row">
                    ...
                    <div class="edit-field-group">
                        ...
                        <input type="hidden" name="gravadora_id" id="edicaoGravadoraId" value="<?= htmlspecialchars($album['gravadora_id'] ?? '') ?>">
                    ...
                    <div class="edit-field-group">
                        ...
                        <select name="tipo_id" id="edicaoTipo" class="<?= !empty($erroValidacao['tipo_id']) ? 'field-error' : '' ?>" required>
                            ...
                            <?php foreach ($tipos as $t): ?>
                                <option value="<?= (int)$t['tipo_id'] ?>" <?= ((isset($album['tipo_id']) && (int)$album['tipo_id'] === (int)$t['tipo_id']) ? 'selected' : '') ?>>
                ...
                <div class="edit-modal-row">
                    <div class="edit-field-group">
                        ...
                        <div class="tags-container" id="containerGeneros">
                            <?php foreach (explode('|', $album['generos'] ?? '') as $gen): if (empty(trim($gen))) continue; ?>
                                <span class="tag-item">
                    ...
                    <div class="edit-field-group">
                        ...
                        <div class="tags-container" id="containerEstilos">
                            <?php foreach (explode('|', $album['estilos'] ?? '') as $est): if (empty(trim($est))) continue; ?>
                                <span class="tag-item">
                ...
                <div class="edit-modal-row">
                    <div class="edit-field-group">
                        ...
                        <div class="tags-container" id="containerProdutores">
                            <?php foreach (explode('|', $album['produtores'] ?? '') as $prod): if (empty(trim($prod))) continue; ?>
                                <span class="tag-item">
                ...
                <div class="edicao-section-faixas">
                    ...
                    <div id="containerListaFaixas" class="lista-faixas-edicao">
                        ...
                        <div id="corpoListaFaixas"></div>
                    </div>
                </div>

                <div class="edicao-actions" style="margin-top:30px; display:flex; justify-content: flex-end; gap:10px;">
                    <button type="submit" class="btn" style="background-color: var(--action-positive);"><i class="fa-solid fa-check"></i> CADASTRAR ÁLBUM</button>
                    <a href="index.php?url=loja" class="btn" style="background-color: var(--action-destructive); text-decoration: none; text-align: center;">Desistir</a>
                </div>
            </div>
        </form>
    </div>
