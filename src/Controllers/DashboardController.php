...
class DashboardController {
    ...
    public function index() {
        $sugestaoDoDia = null;
        $cookieName = 'soundhaven_sugestao_diaria';

        if (!isset($_COOKIE[$cookieName])) {
            // Correção arquitetural: Agora busca direto do DashboardRepository usando a conexão correta
            $sugestaoDoDia = $this->repository->buscarSugestaoAleatoria();

            if ($sugestaoDoDia) {
                $segundosAteMeiaNoite = strtotime('tomorrow') - time();
                setcookie($cookieName, $sugestaoDoDia['album_id'], time() + $segundosAteMeiaNoite, '/');
            }
        }
        ...
        $viewData = [
            ...
            'sugestaoDoDia'     => $sugestaoDoDia // Enviando a pérola com segurança para a View
        ];
